...
from utils.composition import generate_features, _element_composition, _weight_fractionl_composition, _electroneg_fractionl_composition
...
class EDMDataset(Dataset):
    ...
    def __init__(self, dataset, n_comp):
        ...
        self.y = np.array(self.data[1]) # y is target 
        self.formula = np.array(self.data[2]) # formula
...
def get_edm(path, elem_prop='mat2vec', n_elements='infer',
    ...
    if 'formula' not in df.columns.values.tolist():  
        df['formula'] = df['cif_id'].str.split('_ICSD').str[0] # ['formula', 'target']

    ####################################### elements weight fraction ##########################################
    df['count'] = [len(_weight_fractionl_composition(form)) for form in df['formula']]
    ...
    ####################################### elements weight fraction ##########################################
    list_ohm = [OrderedDict(_weight_fractionl_composition(form)) 
                for form in df['formula']]
    # list_ohm = [OrderedDict(_electroneg_fractionl_composition(form)) 
    #             for form in df['formula']]
    list_weight_ohm = [OrderedDict(sorted(mat.items(), key=lambda x:-x[1]))
                for mat in list_ohm]
    ...
    elem_num = np.zeros(shape=(len(list_weight_ohm), n_elements), dtype=data_type_np) # (239125, 16)
    elem_weight_frac = np.zeros(shape=(len(list_weight_ohm), n_elements), dtype=data_type_np) # (239125, 16)

    #####################################weight ################################################################
    for i, comp in enumerate(tqdm(list_weight_ohm,
                                  desc="Generating weight EDM",
                                  unit="formulae",
                                  disable=not verbose)):
        for j, (elem, count) in enumerate(list_weight_ohm[i].items()):
            if j == n_elements:
                # Truncate EDM representation to n_elements
                break
            try:
                elem_weight_frac[i, j] = float(count)
    ...
    #####################################weight ################################################################
    if n_elements == 16:
        n_elements = np.max(np.sum(elem_weight_frac > 0, axis=1, keepdims=True))
        elem_num = elem_num[:, :n_elements]
        elem_weight_frac = elem_weight_frac[:, :n_elements] # 每个elem的分数占比 (239125, 7)

    elem_num = elem_num.reshape(elem_num.shape[0], elem_num.shape[1], 1)
    elem_weight_frac = elem_weight_frac.reshape(elem_weight_frac.shape[0], elem_weight_frac.shape[1], 1)
    out = np.concatenate((elem_num, elem_weight_frac), axis=1)

    # print(elem_num.shape)
    # print(elem_weight_frac.shape)
...
class EDM_CsvLoader():
    ...
    ----------
    csv_data: str
        name of csv file containing cif(not include structure only composition formula) and properties
    csv_val: str
