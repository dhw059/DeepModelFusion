...
from lgdcnn.utils.composition import generate_features, _element_composition, _weight_fractionl_composition, _electroneg_fractionl_composition
...
def get_edm(path, elem_prop='mat2vec', n_elements='infer',
    ...
    # df = pd.concat([df['formula'], df['band_gap']], axis=1, ignore_index=False)
    ...
    # print(elem_num.shape, elem_frac.shape)
    # print(elem_num, elem_frac)
...
class RobustLoss(nn.Module):
    ...
    def forward(self, y_true, y_pred):
        mean, sigma = torch.chunk(y_pred, 2, dim=-1)
        sigma = torch.clamp(sigma, -10.0, 10.0)  # evade exploding gradient
        loss = torch.sqrt(torch.tensor(2.0)) * torch.abs(mean - y_true) * torch.exp(-sigma) + sigma
...
class RobustMAE():
    def __init__(self, scaler, pred_func=False):
        self.scaler = scaler
    ...
    def mean_absolute_error(self, y_true, y_pred):
        ...
        mean, sigma = torch.chunk(y_pred, 2, dim=-1)
        if self.pred_func:
            mae = torch.abs(mean - y_true)
        else:
            # mae = torch.abs(self.scaler.unscale(mean) - y_true)
            mae = torch.abs(self.scaler.denorm(mean) - self.scaler.denorm(y_true))
