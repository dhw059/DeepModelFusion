...
def get_edm(path, elem_prop='mat2vec', n_elements='infer',
    ...
    if isinstance(path, str):
        df = pd.read_csv(path, keep_default_na=False, na_values=[''])
            ...
            df = path
    df = df.rename(columns={"X_O": "O", "X_V": "V"})
    # df = df.rename(columns={"X_O": "O", "X_Li": "Li", "X_Cu":"Cu"})
    ...
    k = ['O','V']
    # k = ['O','Li','Cu']
    dic_compostion = [dict(zip(k,i)) for i in df[['O','V']].values.tolist()]
    # dic_compostion = [dict(zip(k,i)) for i in df[['O','Li','Cu']].values.tolist()]

    list_ohm = [OrderedDict(form) for form in dic_compostion]


    y = df['delta_e'].values.astype(data_type_np)

    formula = [Composition.from_dict(i).reduced_formula for i in dic_compostion]
    # print(formula)
    # print(len(formula))
