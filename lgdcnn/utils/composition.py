...
def _weight_fractionl_composition(formula):
    comp = Composition(formula)
    com_dict = comp.get_el_amt_dict()
    weight_dict = {}
    for k, v in com_dict.items():
            if abs(v) >= 1e-6:
                weight_dict[k] = comp.get_wt_fraction(k)
    return weight_dict

def _electroneg_fractionl_composition(formula):
    comp = Composition(formula)
    com_dict = comp.get_el_amt_dict()
    electroneg_dict = {}
    for k, v in com_dict.items():
            if abs(v) >= 1e-6:
                electroneg_dict[k] = comp.get_electroneg_fraction(k)
    return electroneg_dict
