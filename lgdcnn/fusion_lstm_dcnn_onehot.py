...
class multi_lstm_dpcnnmodify(nn.Module):
    ...
    def forward(self, x):
        ...
        output, _ = self.rnn(output1)
        energy = self.projection(output)
        alpha = F.softmax(energy, dim=1)
        out = output * alpha 
        output2 = output + out 

        embedding = output2.permute(0, 2, 1)   
...
class BaseModel(nn.Module):
    def __init__(self, d_model, frac=False, compute_device=None,):
        ...
        self.embed = ElementEmbedder(embedding_size=self.d_model, compute_device=self.compute_device,Emebedder_Choice=False)
